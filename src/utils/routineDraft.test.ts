...
describe('addItem', () => {
  test('añade al final con las series y repeticiones por defecto', () => {
    ...
    expect([DEFAULT_SETS, DEFAULT_REPS]).toEqual([3, 10]);
  });
...
describe('toRoutineInput y fromRoutine', () => {
  test('toRoutineInput conserva el orden y recorta el nombre', () => {
    const input = toRoutineInput('  Día 1 ', setTarget(threeItems(), 'b', 'targetSets', 4));
    expect(input).toEqual({
      name: 'Día 1',
      exercises: [
        { exerciseId: 1, targetSets: 3, targetReps: 10 },
        { exerciseId: 2, targetSets: 4, targetReps: 10 },
        { exerciseId: 1, targetSets: 3, targetReps: 10 },
      ],
    });
  });
  ...
  test('fromRoutine convierte una rutina guardada con claves distintas por fila', () => {
    const routine: Routine = {
      ...
      exercises: [
        { id: 10, exerciseId: 2, exerciseName: 'Sentadilla', exerciseNameEn: null, position: 0, targetSets: 5, targetReps: 5 },
        { id: 11, exerciseId: 1, exerciseName: 'Press banca', exerciseNameEn: 'Bench Press', position: 1, targetSets: 3, targetReps: 8 },
      ],
    };
    let counter = 0;
    ...
    expect(toRoutineInput(routine.name, items).exercises).toEqual([
      { exerciseId: 2, targetSets: 5, targetReps: 5 },
